...
const WHATSAPP_NUMBER = "917300548523";
...
const PICKUP_LOCATIONS = [
  ...
  },
  {
    ...
    icon: "🏯",
    tiers: [
      { maxPeople: 2, totalCost: 3000 },
      { maxPeople: 4, totalCost: 4500 },
      { maxPeople: 7, totalCost: 6000 },
      { maxPeople: 99, totalCost: 8000 },
    ] as SurchargeTier[],
  },
  {
...
function getLocationSurchargePerPerson(
    ...
    const totalCost = tier ? tier.totalCost : loc.tiers[loc.tiers.length - 1].totalCost;
...
export default function CustomPackagePage() {
    ...
    }, [selectedPackage, people, pickupLocation, needDrop, dropLocation, nightStay, food]);
    ...
    const handleSubmit = () => {
        ...
        const pickupLabel =
            PICKUP_LOCATIONS.find((l) => l.id === pickupLocation)?.label ?? pickupLocation;
        const dropLabel = needDrop && dropLocation
            ? (PICKUP_LOCATIONS.find((l) => l.id === dropLocation)?.label ?? dropLocation)
            : "Not Required";
            ...
            <section className="relative h-72 flex items-center justify-center text-white overflow-hidden">
                ...
                {[...Array(10)].map((_, i) => (
                    <motion.div
                        ...
                        transition={{ duration: 2.5 + (i % 3), repeat: Infinity, delay: i * 0.4 }}
                    />
                ))}
                <motion.div
                                    ...
                                    return (
                                        <React.Fragment key={s.id}>
                                            <motion.div
                                                ...
                                                animate={{ opacity: isCurrent || isCompleted ? 1 : 0.45 }}
                                            >
                                                <motion.div
                                                    animate={{
                                                ...
                                                <span
                                                    ...
                                                    style={{ color: isCurrent ? "#ff9933" : isCompleted ? "#16a34a" : "var(--text-muted)" }}
                                                >
                                                    {s.title}
                                                </span>
                                            </motion.div>
                        ...
                        <AnimatePresence mode="wait">
                            {submitted ? (
                                ...
                                >
                                    ...
                                    <h2 className="text-3xl font-serif font-bold" style={{ color: "var(--text)" }}>
                                        Request Sent! 🙏
                                    </h2>
                                    <p className="text-lg" style={{ color: "var(--text-muted)" }}>
                                        Jai Shri Krishna! Your custom package request has been sent via WhatsApp. We'll confirm your booking within a few hours.
                                    </p>
                                    <button
                                        onClick={() => { setSubmitted(false); setStep(1); setSelectedPackageSlug(""); }}
                                    ...
                                    <div className="p-6 md:p-8">
                                        ...
                                        {step === 1 && (
                                            <div>
                                                ...
                                                <p className="text-sm mb-6" style={{ color: "var(--text-muted)" }}>
                                                    Choose one of our existing packages as your starting point. You'll customise it in the next steps.
                                                </p>
                                                    ...
                                                    {packages.map((pkg) => {
                                                        ...
                                                        return (
                                                            <motion.button
                                                                ...
                                                                style={{
                                                                    borderColor: isSelected ? "#ff9933" : "var(--border)",
                                                                ...
                                                                <div className="flex items-start justify-between gap-2">
                                                                    <div className="flex-1">
                                                                        ...
                                                                        <div className="flex items-center gap-3 text-xs" style={{ color: "var(--text-muted)" }}>
                                                                            ...
                                                                            <span>|</span>
                                                                            <span>from ₹{pkg.price.toLocaleString("en-IN")}/person</span>
                                                                        </div>
                                                                    </div>
                                                                    {isSelected && (
                                                                        <motion.div
                                        ...
                                        {step === 2 && (
                                            <div>
                                                ...
                                                <p className="text-sm mb-6" style={{ color: "var(--text-muted)" }}>
                                                    Where should we pick you up from? A surcharge applies for locations outside Mathura area.
                                                </p>
                                                    ...
                                                    {PICKUP_LOCATIONS.map((loc) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                {(() => {
                                                                    const pp = getLocationSurchargePerPerson(loc.id, people);
                                                                    return (
                                                                        <div className="text-xs mt-1" style={{ color: pp === 0 ? "#22c55e" : "#ff9933" }}>
                                                                            {pp === 0
                                                                                ? "No extra charge"
                                                ...
                                                <div className="flex gap-4 mb-6">
                                                    {[
                                                        ...
                                                        <motion.button
                                                            ...
                                                            style={{
                                                                borderColor: needDrop === val ? "#ff9933" : "var(--border)",
                                                                background: needDrop === val ? "rgba(255,153,51,0.1)" : "var(--surface-2)",
                                                                color: needDrop === val ? "#ff9933" : "var(--text-muted)",
                                                            }}
                                                        >
                                                            {needDrop === val ? "✓ " : ""}{label}
                                                        </motion.button>
                                            ...
                                            <div>
                                                ...
                                                <div className="space-y-5">
                                                    ...
                                                    <div
                                                        ...
                                                        style={{ background: "var(--surface-2)", border: `1px solid ${nightStay ? "#ff9933" : "var(--border)"}` }}
                                                        onClick={() => setNightStay((v) => !v)}
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <Moon className={`w-5 h-5 ${nightStay ? "text-saffron" : "text-gray-400"}`} />
                                                            <div>
                                                                <span className="font-bold" style={{ color: "var(--text)" }}>
                                                                    Night Stay
                                                                </span>
                                                        ...
                                                        <motion.div
                                                            animate={{ background: nightStay ? "linear-gradient(135deg, #ff9933, #ffd700)" : "var(--border)" }}
                                                    ...
                                                    <div
                                                        className="p-5 rounded-xl flex items-center justify-between cursor-pointer"
                                                        style={{ background: "var(--surface-2)", border: `1px solid ${food ? "#ff9933" : "var(--border)"}` }}
                                                        onClick={() => setFood((v) => !v)}
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <Utensils className={`w-5 h-5 ${food ? "text-saffron" : "text-gray-400"}`} />
                                                            <div>
                                                                <span className="font-bold" style={{ color: "var(--text)" }}>
                                                                    Food Included
                                                                </span>
                                                                <p className="text-xs" style={{ color: "var(--text-muted)" }}>
                                                                    +₹400/person/day (vegetarian meals)
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <motion.div
                                                            animate={{ background: food ? "linear-gradient(135deg, #ff9933, #ffd700)" : "var(--border)" }}
                                                            className="w-12 h-6 rounded-full flex items-center px-1"
                                                        >
                                                            <motion.div
                                                                animate={{ x: food ? 24 : 0 }}
                                        ...
                                        {step === 5 && (
                                            <div>
                                                ...
                                                <div className="space-y-5">
                                                    <div>
                                                        <label className={`${labelClass} flex items-center gap-2`} style={{ color: "var(--text-muted)" }}>
                                                    ...
                                                    >
                                                        By submitting, your enquiry will be sent as a WhatsApp message to our team at +91 73005 48523. We'll confirm your custom package within a few hours. 🙏
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                            ...
                                            {step < 5 ? (
                                                <motion.button
                                                    ...
                                                    style={{
                                                        ...
                                                        boxShadow: canGoNext() ? "0 4px 15px rgba(255,153,51,0.4)" : "none",
                                                    }}
                                                >
                                                    {step === 3 ? (needDrop ? "Choose Drop City →" : "Skip Drop →") : "Next"}
                                                ...
                                                <motion.button
                                                    ...
                                                    style={{
                                                        background: canGoNext()
                                                            ...
                                                            : "rgba(0,0,0,0.2)",
                                                        cursor: canGoNext() ? "pointer" : "not-allowed",
                                                        boxShadow: canGoNext() ? "0 4px 20px rgba(37,211,102,0.4)" : "none",
                                                    }}
                                                >
                                    ...
                                    <AnimatePresence>
                                        ...
                                        >
                                            ...
                                            <div
                                                ...
                                                style={{ background: "rgba(255,153,51,0.07)", border: "1px solid rgba(255,153,51,0.15)" }}
                                            >
                                                <div className="text-xs font-semibold text-saffron mb-0.5">Selected Package</div>
                                                ...
                                                <div className="space-y-2 text-sm">
                                                    <div className="flex justify-between" style={{ color: "var(--text-muted)" }}>
                                                        ...
                                                        <span className="font-semibold" style={{ color: "var(--text)" }}>
                                                            ₹{priceBreakdown.basePerPerson.toLocaleString("en-IN")}
                                                        </span>
                                                    </div>
                                                    {priceBreakdown.pickupSurcharge > 0 && (
                                                        <div className="flex justify-between" style={{ color: "var(--text-muted)" }}>
                                                            ...
                                                            <span className="font-semibold text-amber-500">
                                                                +₹{priceBreakdown.pickupSurcharge.toLocaleString("en-IN")}
                                                            </span>
                                                        </div>
                                                    )}
                                                    {priceBreakdown.dropSurcharge > 0 && (
                                                        <div className="flex justify-between" style={{ color: "var(--text-muted)" }}>
                                                            ...
                                                            <span className="font-semibold text-amber-500">
                                                                +₹{priceBreakdown.dropSurcharge.toLocaleString("en-IN")}
                                                            </span>
                                                        </div>
                                                    )}
                                                    {priceBreakdown.nightStayCost > 0 && (
                                                        <div className="flex justify-between" style={{ color: "var(--text-muted)" }}>
                                                            ...
                                                            <span className="font-semibold text-amber-500">
                                                                +₹{priceBreakdown.nightStayCost.toLocaleString("en-IN")}
                                                            </span>
                                                        </div>
                                                    )}
                                                    {priceBreakdown.foodCost > 0 && (
                                                        <div className="flex justify-between" style={{ color: "var(--text-muted)" }}>
                                                            ...
                                                            <span className="font-semibold text-amber-500">
                                                                +₹{priceBreakdown.foodCost.toLocaleString("en-IN")}
                                                            </span>
                                                        </div>
                                                    )}
                                                    ...
                                                    >
                                                        <div className="flex justify-between text-sm" style={{ color: "var(--text-muted)" }}>
                                                            ...
                                                            <span className="font-bold" style={{ color: "var(--text)" }}>
                                                                ₹{priceBreakdown.perPersonTotal.toLocaleString("en-IN")}
                                                            </span>
                                                        </div>
                                                    ...
                                                    >
                                                        ...
                                                        <div className="text-xs mt-1" style={{ color: "var(--text-muted)" }}>
                                                            for {people} person{people !== 1 ? "s" : ""} · {selectedPackage.duration}
                                                        </div>
                                                    </motion.div>

                                                    <p className="text-[11px] italic text-center" style={{ color: "var(--text-subtle)" }}>
